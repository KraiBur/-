...
<img src="%s" alt="">
		...
		', $row["path"], $row["product_id"], $row["name"],   $row["product_id"],$row["year"],$row["category"],  
		($role == "admin") ? '
			...
			' : '',
		($role != "guest") ? '<div class="cart__mini"> <p class="text-right"><a href="controllers/add_cart.php?id='. $row["product_id"] .'" class="text-small">В избранное</a></p></div>' : '');

	if($data == "")
		$data = '<h3 class="text-center"> отсутствуют</h3>';
?>
<div class="content">
